  ...
  <AdminShell active="users" title="用户详情" subtitle="查看账号、次数和订单统计">
    ...
    <template v-else-if="user">
      ...
      <view class="stats-grid">
        <view><text>当前余额</text><text>{{user.balance}}</text><text>次</text></view>
...
import { AlertCircle, ArrowLeft, ArrowRight, BadgePlus, LoaderCircle, UserCheck, UserRound, UserX } from '@lucide/vue'
...
const statusModal=ref(false);const creditModal=ref(false);const statusReason=ref('');const creditForm=reactive({change_count:'',reason:''});const modalError=ref('');const submitting=ref(false)
const canManage=computed(()=>user.value&&user.value.role!=='admin'&&user.value.id!==adminStore.profile?.id)
const previewBalance=computed(()=>{const count=normalizeInteger(creditForm.change_count);return (user.value?.balance||0)+(count??0)})
...
function openCreditModal(){creditForm.change_count='';creditForm.reason='';modalError.value='';creditModal.value=true}
