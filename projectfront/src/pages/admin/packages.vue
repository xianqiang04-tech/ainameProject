  ...
  <AdminShell active="packages" title="套餐管理" subtitle="维护套餐价格、次数和启用状态">
    ...
    <view class="admin-data-panel">
      ...
      <view v-else class="admin-table-scroll"><view class="admin-table packages-table">
        <view class="admin-table-row header"><view class="admin-table-cell">套餐</view><view class="admin-table-cell">类型</view><view class="admin-table-cell">价格</view><view class="admin-table-cell">包含次数</view><view class="admin-table-cell">状态</view><view class="admin-table-cell">创建时间</view><view class="admin-table-cell"></view></view>
        <view v-for="item in result.items" :key="item.id" class="admin-table-row">
          ...
          <view class="admin-table-cell" data-label="类型"><text>{{item.type==='logo'?'Logo 次数包':'起名套餐'}}</text></view>
    ...
    <AdminModal :open="modalOpen" :title="editingId?'编辑套餐':'新增套餐'" :subtitle="editingId?'修改内容会记录到审计日志':'创建一个可供用户购买的套餐'" @close="closeModal">
      <view class="admin-form-grid">
        ...
        <view class="admin-form-field full"><text class="admin-form-label">套餐类型</text><picker :range="typeOptions" range-key="label" :value="typeIndex" @change="changeFormType"><view class="admin-select"><text>{{typeOptions[typeIndex].label}}</text><ChevronDown :size="15"/></view></picker><text class="admin-form-hint">起名套餐充入起名额度，Logo 次数包充入 Logo 次数</text></view>
...
const modalOpen=ref(false);const editingId=ref(null);const submitting=ref(false);const modalError=ref('');const form=reactive({name:'',price:'',credit_count:'',type:'name',is_active:true,reason:''})
const typeOptions=[{label:'起名套餐',value:'name'},{label:'Logo 次数包',value:'logo'}]
const typeIndex=computed(()=>Math.max(0,typeOptions.findIndex(option=>option.value===form.type)))
function changeFormType(event){form.type=typeOptions[Number(event.detail.value)].value}
...
function resetForm(){Object.assign(form,{name:'',price:'',credit_count:'',type:'name',is_active:true,reason:''});modalError.value=''}
function openCreate(){editingId.value=null;resetForm();modalOpen.value=true}
function openEdit(item){editingId.value=item.id;Object.assign(form,{name:item.name,price:String(item.price),credit_count:String(item.credit_count),type:item.type||'name',is_active:item.is_active,reason:''});modalError.value='';modalOpen.value=true}
...
async function submit(){const message=validate();if(message){modalError.value=message;return}submitting.value=true;modalError.value='';const payload={name:form.name.trim(),price:Number(form.price).toFixed(2),credit_count:Number(form.credit_count),type:form.type,is_active:form.is_active,reason:form.reason.trim()||null};try{if(editingId.value)await adminApi.updatePackage(editingId.value,{...payload,reason:form.reason.trim()});else await adminApi.createPackage(payload);modalOpen.value=false;uni.showToast({title:editingId.value?'套餐已更新':'套餐已创建',icon:'success'});await loadPackages()}catch(error){modalError.value=error.message}finally{submitting.value=false}}
</script>

<style scoped>
.packages-table{min-width:1010px}.packages-table .admin-table-row{grid-template-columns:minmax(190px,1.6fr) .75fr .8fr .8fr .8fr 1.25fr .6fr}.switch-line{display:flex;min-height:40px;align-items:center;gap:12px;color:#68756f;font-size:12px}.admin-form-hint{display:block;margin-top:8px;color:#89918d;font-size:11px}@media(max-width:760px){.packages-table{min-width:0}}
</style>
