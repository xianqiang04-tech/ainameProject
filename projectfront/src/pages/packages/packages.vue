<template>
  <view class="packages-page page-shell">
    <view class="packages-header page-content">
      ...
      <view><text class="section-title">{{activeTab==='logo'?'选择 Logo 次数包':'选择起名套餐'}}</text><text class="section-note">{{activeTab==='logo'?'购买成功后，Logo 次数会自动加入当前账户。':'购买成功后，起名额度会自动加入当前账户。'}}</text></view>
      <view class="current-balance"><text>{{activeTab==='logo'?'Logo 次数':'当前额度'}}</text><text>{{currentCount}} 次</text></view>
    </view>
    <view class="package-tabs page-content">
      <view class="package-tab" :class="{active:activeTab==='name'}" @click="switchTab('name')">起名套餐</view>
      <view class="package-tab" :class="{active:activeTab==='logo'}" @click="switchTab('logo')">Logo 次数包</view>
    </view>
    <view class="page-content">
      ...
      <view v-else class="package-grid">
        <view v-for="(item,index) in packages" :key="item.id" class="package-card" :class="{featured:index===1}">
          ...
          <view class="credit-count"><text>{{item.credit_count}}</text><text>{{unitLabel}}</text></view>
          <view class="price"><text>¥</text><text>{{formatPrice(item.price)}}</text></view>
          <view class="package-points"><view><Check :size="16"/><text>购买后立即到账</text></view><view><Check :size="16"/><text>{{activeTab==='logo'?'用于企业 Logo 生成':'用于首次起名和反馈调整'}}</text></view></view>
...
<script setup>
import {computed,ref} from 'vue'; import {onLoad} from '@dcloudio/uni-app'; import {ArrowLeft,LoaderCircle,PackageOpen,Check,ShoppingBag,ShieldCheck} from '@lucide/vue'; import {packageApi} from '../../api'; import {useAuthStore} from '../../stores/auth'; import {requireAuth} from '../../utils/storage'
const authStore=useAuthStore(); const packages=ref([]); const loading=ref(true); const errorMessage=ref(''); const payingId=ref(null); const activeTab=ref('name')
const currentCount=computed(()=>activeTab.value==='logo'?(authStore.logoBalance===null?'--':authStore.logoBalance):(authStore.balance===null?'--':authStore.balance))
const unitLabel=computed(()=>activeTab.value==='logo'?'次 Logo 设计次数':'次起名额度')
onLoad(async(query)=>{if(!requireAuth())return;if(query&&query.tab==='logo')activeTab.value='logo';try{await authStore.refreshBalance()}catch(_){}await loadPackages()})
...
function switchTab(tab){if(activeTab.value===tab||loading.value)return;activeTab.value=tab;loadPackages()}
async function loadPackages(){loading.value=true;errorMessage.value='';try{packages.value=await packageApi.list(activeTab.value)}catch(error){errorMessage.value=error.message}finally{loading.value=false}}
async function selectPackage(item){payingId.value=item.id;errorMessage.value='';try{const detail=await packageApi.detail(item.id);uni.showModal({title:`购买 ${detail.name}`,content:`支付 ¥${formatPrice(detail.price)}，获得 ${detail.credit_count} ${unitLabel.value}。确认后将跳转支付宝。`,confirmText:'确认购买',success:async({confirm})=>{if(!confirm){payingId.value=null;return}try{const order=await packageApi.createOrder(detail.id);
  // #ifdef H5
